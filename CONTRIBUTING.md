...
| Field | Contract |
|---|---|
| `display_order` | Optional numeric position within the module catalog. It defaults to the trailing number in the activity id and must stay unique within a module — the build fails on duplicates. Catalog, builder, and module pages lead with the module's primary outcome journey, so `display_order` only orders activities outside that journey. Set it only when stable activity IDs do not match the intended presentation order. |
