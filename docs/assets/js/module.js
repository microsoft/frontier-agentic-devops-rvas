...
(function () {
    ...
    list.innerHTML = mod.tracks.map((t) => {
      ...
      return `
        <${tag} class="track-item"${href} style="--mod-color:${color}">
          ...
          <div class="track-item-count">${count} work package${count === 1 ? '' : 's'}</div>
        </${tag}>`;
    }).join('');
  }
  ...
  function renderChallenges(challenges, mod) {
    ...
    if (!challenges.length) {
      grid.innerHTML = '<div class="empty">No work packages found for this module.</div>';
      return;
    }
    ...
    order.forEach((trackId) => {
      ...
      html += `
        <div class="group-head" id="track-${trackId}" style="--mod-color:${color}">
          ...
          <span class="group-count">${items.length} work package${items.length === 1 ? '' : 's'}</span>
        </div>
