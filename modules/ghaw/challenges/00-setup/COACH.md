...
- Surface and document access blockers early; apply fallback paths before Activity 1-01.
...
- Customer delivery team members do not need write access to `microsoft/frontier-agentic-devops-rvas` for the trial smoke test. `gh-aw` creates or uses a trial host repository in the delivery team member's own GitHub account and only simulates the delivery session repository as the target.
...
| Cannot write to `microsoft/frontier-agentic-devops-rvas` | Expected for customer delivery team members using the public repo | No action needed for the smoke test; trial mode writes to the delivery team member's trial host repository, not the delivery session repo |
...
Before releasing the group to Activity 1-01, confirm per participant:
