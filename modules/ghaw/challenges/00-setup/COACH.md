...
- If `gh aw trial ... --dry-run` fails with "failed to determine simulated host repository", make sure the command includes `--logical-repo microsoft/frontier-agenticdevops-hackathon`. This avoids relying on the participant's local Git remote URL, which may use a custom SSH host alias.
- Students do not need write access to `microsoft/frontier-agenticdevops-hackathon` for the trial smoke test. `gh-aw` creates or uses a trial host repository in the student's own GitHub account and only simulates the hackathon repository as the target.
...
| `gh aw trial modules/ghaw/resources/examples/hello-world.md --logical-repo microsoft/frontier-agenticdevops-hackathon --dry-run --yes` fails with API key error | AI engine key missing | Confirm Copilot subscription is active; or set `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY` as Codespace secrets and rebuild |
| `gh aw trial ... --dry-run` fails with `failed to determine simulated host repository` | Local Git remote uses a custom SSH host alias or another URL `gh-aw` cannot parse | Re-run with `--logical-repo microsoft/frontier-agenticdevops-hackathon`; do not ask participants to change remotes unless they need to |
| Cannot write to `microsoft/frontier-agenticdevops-hackathon` | Expected for students using the public repo | No action needed for the smoke test; trial mode writes to the student's trial host repository, not the hackathon repo |
| Repository not accessible | Not in the right repo or public repo access is blocked | Confirm the Codespace or local clone is for `microsoft/frontier-agenticdevops-hackathon` |
...
- [ ] `gh aw trial modules/ghaw/resources/examples/hello-world.md --logical-repo microsoft/frontier-agenticdevops-hackathon --dry-run --yes` completes without errors
