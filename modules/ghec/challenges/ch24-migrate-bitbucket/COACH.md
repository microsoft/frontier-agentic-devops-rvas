...
This challenge teaches learners to choose the correct Bitbucket migration path instead of treating all Bitbucket sources as equivalent. The primary hands-on path is Bitbucket Server/Data Center to GitHub Enterprise Cloud with `gh bbs2gh`, preserving Git history and pull request metadata. The fallback path is Bitbucket Cloud to GitHub using `git clone --mirror` and `git push --mirror`, preserving only source and Git history.
...
**Symptom:** Learners ask for a `bbs2gh` command for bitbucket.org or expect PRs to appear after a mirror push.  
...
Use these checks as prompts; adapt names to the learner's repositories.
...
For Server/Data Center, UI inspection is required for PR metadata. Ask the learner to open migrated pull requests and show comments, reviews, file/line comments, required reviewers, and attachments.
