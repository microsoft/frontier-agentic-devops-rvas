...
- `migrate-ado-repos.ps1` exists and the customer implementation owner can explain what repos it queues or migrates.
...
- Migration logs are downloaded within 24 hours, or the customer implementation owner has the migration ID and exact log-download command.
...
Ask the customer implementation owner to show the source ADO PR and the migrated GitHub PR side by side. If work item links existed on the source PR, verify the link is visible on the migrated PR.
