...
#
# challenges/04-actions-ci-fundamentals/provision.sh
#
...
# failing test (set GHEC_FAIL=1 to make CI go red — for practising red/green
...
"# ghec-${CHID} — GitHub Actions CI Fundamentals
...
"
  ...
  gh_put_file "$ORG" "$REPO" "test/app.test.js" "seed test suite (ghec-${CHID})" \
...
"
  ...
  gh_put_file "$ORG" "$REPO" ".github/workflows/ci.yml" "seed echo-only starter workflow (ghec-${CHID})" \
"name: ci
# ghec-${CHID} STARTER — echo only. Replace with a real pipeline:
...
ghec_provision() {
  ...
  log_info "  - flip GHEC_FAIL=1 to watch the gate go red, then green again"
}
