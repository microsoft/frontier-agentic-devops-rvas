...
R_TARGET="wth-${CHID}-audit-target"
TEAM="wth-${CHID}-auditors"

_ch09_seed_target() {
  gh_put_file "$ORG" "$R_TARGET" "README.md" "seed README (wth-${CHID})" \
"# ${R_TARGET}

Seeded by wth-${CHID} (Audit Log Streaming). Activity here (pushes, team
changes, settings edits) shows up in the org audit log. Set up streaming and
verify these events land in your sink."
  gh_put_file "$ORG" "$R_TARGET" "src/index.js" "seed src (wth-${CHID})" \
"console.log('audit target — wth-${CHID}');
"
}
...
wth_provision() {
  ...
  gh_create_team "$ORG" "$TEAM" "wth-${CHID} auditors team"
